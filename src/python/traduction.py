...
class PlotLearning(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        ax1.set_yscale('log')
        ax1.plot(self.x, self.losses, label="train_loss")
        ...
        path = "models/en-" + str(FLAGS.data_path).split("/")[-1].replace(".txt", "/")
        if not os.path.exists(path):
            os.makedirs(path)

        plt.savefig(path + '/logs.png')
...
def main():
  ...
  tbCallBack = callbacks.TensorBoard(log_dir=path + '/Graph', histogram_freq=0, write_graph=True, write_images=True)
  earlStopCallBack = callbacks.EarlyStopping(monitor='val_loss',min_delta=0,patience=0, verbose=0, mode='auto')
  ckptCallBack = callbacks.ModelCheckpoint(path + 'model_best.h5', verbose=1, monitor='val_acc',save_best_only=True, mode='auto')
  ...
  model.fit([encoder_input_data, decoder_input_data], decoder_target_data,
            ...
            epochs=FLAGS.epochs,
            callbacks=[tbCallBack , ckptCallBack, earlStopCallBack, plot_losses],
            validation_split=0.2)

  model = load_model(path + "model_best.h5")
  tfjs.converters.save_keras_model(model, path)

  if not FLAGS.include_test:
      print("Done.")

      with open(path + "/time.txt", "w") as f:
          f.write("Elapsed time: " + str(time.time() - t1))
